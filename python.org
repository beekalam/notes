...
** Python 3.0 division operators
Python 3 has two division operators.

The / true division operator always tries to produce a true, floating-point
...
The // truncated division operator always tries to produce a truncated result.
...
By default, Python 2 only has one division operator. For programmers still using
Python 2, we can start using these new division operators with this:
